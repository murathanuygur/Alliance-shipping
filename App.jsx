...
async function supabaseUploadFile(accessToken, file) {
  const path = `${Date.now()}-${file.name}`.replace(/[^a-zA-Z0-9._-]/g, "_");
  const res = await fetch(`${SUPABASE_URL}/storage/v1/object/documents/${encodeURIComponent(path)}`, {
    method: "POST",
    headers: {
...
const SESSION_STORAGE_KEY = "alliance_session";

function saveSessionToStorage(authData) {
  try {
    localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify({ refresh_token: authData.refresh_token }));
  } catch (e) {
    // localStorage unavailable — Remember me simply won't persist in this environment.
  }
}
  ...
  return data
    .map((t) => {
      const updates = (t.task_updates || [])
        ...
        .map((u) => ({ id: u.id, author: u.author, date: u.date, text: u.text, createdAt: u.created_at }));
      const lastActivityAt = updates.reduce((max, u) => (u.createdAt > max ? u.createdAt : max), t.created_at);
      return { id: t.id, title: t.title, status: t.status, updates, lastActivityAt };
    })
...
async function supabaseCreateTask(accessToken, title, status) {
  ...
  return { id: data[0].id, title: data[0].title, status: data[0].status, updates: [], lastActivityAt: data[0].created_at };
}
...
async function supabaseAddTaskUpdate(accessToken, taskId, author, date, text) {
  ...
  return { id: data[0].id, author: data[0].author, date: data[0].date, text: data[0].text, createdAt: data[0].created_at };
}
...
function mapQuoteRow(row) {
  return {
    ...
    costItems: row.cost_items || [],
    revenueItems: row.revenue_items || [],
    convertedOperationId: row.converted_operation_id || null,
  };
}

async function supabaseFetchQuotes(accessToken) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/quotations?select=*&order=created_at.desc`, {
    headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}` },
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Could not load quotations.");
  return data.map(mapQuoteRow);
}

async function supabaseCreateQuote(accessToken, q) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/quotations`, {
    ...
    body: JSON.stringify({
      ...
      cost_items: q.costItems,
      revenue_items: q.revenueItems,
      converted_operation_id: q.convertedOperationId,
    }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Could not save the quotation.");
  return mapQuoteRow(data[0]);
}

async function supabaseUpdateQuote(accessToken, id, q) {
...
async function supabaseDeleteQuote(accessToken, id) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/quotations?id=eq.${id}`, {
    method: "DELETE",
    headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}` },
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.message || "Could not delete the quotation.");
  }
}
...
async function supabaseFetchActivities(accessToken) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/finance_activities?select=*&order=date.desc,created_at.desc`, {
    headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}` },
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Could not load finance activity.");
  return data.map(mapActivityRow);
}

async function supabaseCreateActivity(accessToken, a) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/finance_activities`, {
    method: "POST",
    headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json", Prefer: "return=representation" },
    body: JSON.stringify({
      type: a.type,
      ...
      amount: a.amount,
      currency: a.currency,
      usd_amount: a.usdAmount,
      source: a.source,
      operation_id: a.operationId || null,
    }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Could not save the activity.");
  return mapActivityRow(data[0]);
}

async function supabaseDeleteActivity(accessToken, id) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/finance_activities?id=eq.${id}`, {
    method: "DELETE",
    headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}` },
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.message || "Could not delete the activity.");
  }
}
...
function Wordmark({ size = "md" }) {
  ...
  return (
    <img
      src="/logo.png"
      alt="Alliance Shipping & Logistics LLC"
      style={{ height: big ? 140 : 44, width: "auto", display: "block", margin: big ? "0 auto" : 0 }}
    />
  );
}
...
const STICKY_TILT = [-1.4, 1.1, -0.7, 1.6, -1.1, 0.8, -1.6, 1.3];

function TaskCard({ task, onOpen, onDelete, index }) {
  const lastUpdate = task.updates[0];
  const tilt = STICKY_TILT[index % STICKY_TILT.length];
  ...
  return (
    <div
      style={{
        background: "#FFFDF6",
        backgroundImage:
          "repeating-linear-gradient(#FFFDF6 0px, #FFFDF6 27px, #EDE7D6 28px)",
    ...
    >
      <div
        style={{
          ...
          width: 22,
          height: 14,
          background: "rgba(184,147,63,0.55)",
          borderRadius: 2,
      ...
      {lastUpdate ? (
        <div style={{ borderTop: `1px dashed ${s.color}55`, paddingTop: 8 }}>
          <div style={{ fontSize: 12, color: C.slateLight, marginBottom: 3 }}>
            <span style={{ fontWeight: 700, color: C.navyInk }}>{lastUpdate.author}</span> · {formatDate(lastUpdate.date)}
          </div>
          <div
            style={{
              fontSize: 13.5,
        ...
        <div style={{ fontSize: 12.5, color: C.slateLight, fontStyle: "italic" }}>No updates yet.</div>
      )}
    </div>
  );
}
...
function TaskModal({ task, onClose, currentUser, onCreateTask, onSaveFields, onDeleteTask, onAddUpdate, onDeleteUpdate }) {
  const [activeTask, setActiveTask] = useState(task); // null while creating a brand-new task
  ...
  async function save() {
    ...
    try {
      if (isNew) {
        ...
        setActiveTask(created); // switch modal into edit mode so updates can be added right away
      } else {
...
function DocumentsEditor({ documents, setDocuments, accessToken }) {
  ...
  async function handleFile(e) {
    ...
    setUploading(true);
    try {
      const url = await supabaseUploadFile(accessToken, file);
      setDocuments([...documents, { id: Date.now(), type: docType, name: file.name, url }]);
    } catch (err) {
      setError(err.message);
    } finally {
      setUploading(false);
  ...
  return (
    <div>
            ...
            <div key={d.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: `1px solid ${C.border}` }}>
              {d.url ? (
                ...
                >
                  ...
                  <span style={{ fontSize: 14.5, color: C.blue, textDecoration: "underline", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.name}</span>
                </a>
              ) : (
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <label style={{ ...smallDarkButton, marginTop: 0, display: "inline-flex", alignItems: "center", gap: 8, cursor: uploading ? "default" : "pointer", opacity: uploading ? 0.6 : 1 }}>
      ...
      <div style={{ fontSize: 12.5, color: C.slateLight, marginTop: 8 }}>
        Files are stored securely and can be opened by anyone on the team from any device.
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ background: "#FFFFFF", border: `1px solid ${C.border}`, borderRadius: 6, padding: 16 }}>
        ...
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1.5px solid ${C.navyDeep}` }}>
          ...
          {status === "ready" && (usdCost > 0 || usdRevenue > 0) && (
            <div style={{ marginTop: 18 }}>
              <div style={{ fontSize: 12.5, fontWeight: 700, color: C.slate, marginBottom: 4 }}>Revenue vs. Cost</div>
              <div style={{ height: 200 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={[
                        { name: "Revenue", value: usdRevenue },
...
function OperationModal({ op, onClose, onSave, onDelete, onCloseOperation, rates, ratesStatus, accessToken }) {
  const isNew = !op;
  ...
  return (
    <ModalShell onClose={onClose} title={isNew ? "New Operation" : "Operation Details"} onDelete={!isNew ? handleDelete : null}>
      ...
      <DocumentsEditor documents={documents} setDocuments={setDocuments} accessToken={accessToken} />
...
function QuoteModal({ quote, onClose, onSave, onDelete, onConvert, nextNumber, rates, ratesStatus }) {
  ...
  return (
    <ModalShell onClose={onClose} title={isNew ? "New Quotation" : "Quotation Details"} onDelete={!isNew ? handleDelete : null}>
      ...
      <button onClick={save} disabled={saving} style={{ ...primaryButton, opacity: saving ? 0.7 : 1, cursor: saving ? "default" : "pointer" }}>
        {saving ? "Saving..." : isNew ? "Save Quotation" : "Save Changes"}
      </button>
...
function TasksPanel({ currentUser, accessToken }) {
  ...
  async function addUpdate(taskId, date, text) {
    ...
    setTasks((prev) => {
      const updated = prev.map((t) =>
        t.id === taskId ? { ...t, updates: [created, ...t.updates], lastActivityAt: created.createdAt } : t
      );
  ...
  return (
    <>
      <style>{`
        .task-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 24px 20px; }
      ...
      {loading ? (
        ...
        <div className="task-grid">
          {visibleTasks.map((task, i) => (
            <TaskCard key={task.id} task={task} index={i} onOpen={setModalTask} onDelete={deleteTask} />
          ))}
        </div>
      )}
      {modalTask !== undefined && (
...
function QuotesPanel({ accessToken, onConvertToOperation, rates, ratesStatus }) {
  ...
  useEffect(() => {
    let cancelled = false;
    supabaseFetchQuotes(accessToken)
      .then((data) => {
  ...
  async function saveQuote(quoteData) {
    const exists = quotes.some((q) => q.id === quoteData.id);
    if (exists) {
      const updated = await supabaseUpdateQuote(accessToken, quoteData.id, quoteData);
      setQuotes((prev) => prev.map((q) => (q.id === updated.id ? updated : q)));
    } else {
      const created = await supabaseCreateQuote(accessToken, { ...quoteData, quoteNumber: quoteData.quoteNumber || nextQuoteNumber(quotes) });
      setQuotes((prev) => [created, ...prev]);
    }
  }

  async function deleteQuote(id) {
    await supabaseDeleteQuote(accessToken, id);
  ...
  return (
    <>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: C.slateLight, fontSize: 16 }}>Loading quotations...</div>
      ) : loadError ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: C.red, fontSize: 16 }}>{loadError}</div>
      ) : quotes.length === 0 ? (
...
function AddActivityModal({ onClose, onSave, rates, ratesStatus }) {
  ...
  const canSave = category.trim() && amount && Number(amount) > 0 && ratesStatus === "ready" && !saving;
  ...
  return (
    <ModalShell onClose={onClose} title="Add Activity">
      ...
      {error && <div style={{ color: C.red, fontSize: 13.5, marginTop: 12 }}>{error}</div>}
...
function FinancePanel({ accessToken, rates, ratesStatus }) {
  ...
  useEffect(() => {
    let cancelled = false;
    supabaseFetchActivities(accessToken)
      .then((data) => {
  ...
  async function addActivity(activity) {
    const created = await supabaseCreateActivity(accessToken, activity);
    setActivities((prev) => [created, ...prev]);
  }
  async function deleteActivity(id) {
    await supabaseDeleteActivity(accessToken, id);
  ...
  return (
    <>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: C.slateLight, fontSize: 16 }}>Loading finance activity...</div>
      ) : loadError ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: C.red, fontSize: 16 }}>{loadError}</div>
      ) : activities.length === 0 ? (
...
function Dashboard({ session, onLogout }) {
  ...
  async function closeOperationToFinance(opData) {
    ...
    for (const activity of newActivities) {
      await supabaseCreateActivity(session.accessToken, activity);
    }
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.cream, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ background: "#FFFFFF", padding: "16px 24px", borderBottom: `1px solid ${C.border}`, boxShadow: "0 1px 4px rgba(11,31,58,0.05)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            ...
            {NAV_ITEMS.map((item) => (
              <button
                ...
                style={{
                  ...
                  background: activeTab === item.key ? C.gold : item.ready ? "rgba(184,147,63,0.14)" : "transparent",
                  color: activeTab === item.key ? C.navyDeep : item.ready ? C.gold : C.slateLight,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <span style={{ fontSize: 15, color: C.navyDeep, fontWeight: 600 }}>{displayName}</span>
            <button onClick={onLogout} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, color: C.slate, fontSize: 14 }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "36px 24px 60px" }}>
        ...
        {activeTab === "quotes" && (
          <QuotesPanel accessToken={session.accessToken} onConvertToOperation={convertQuoteToOperation} rates={rates} ratesStatus={ratesStatus} />
        )}
        {activeTab === "finance" && (
          <FinancePanel accessToken={session.accessToken} rates={rates} ratesStatus={ratesStatus} />
        )}
...
export default function App() {
  ...
  async function buildSession(authData) {
    let profile = null;
    try {
    ...
    return { accessToken: authData.access_token, refreshToken: authData.refresh_token, authUser: authData.user, profile };
  }
  ...
  useEffect(() => {
    ...
    supabaseRefreshSession(stored.refresh_token)
      .then(async (authData) => {
        saveSessionToStorage(authData); // Supabase rotates the refresh token, so keep storage in sync
        setSession(await buildSession(authData));
      })
      .catch(() => {
  ...
  async function handleLogin(authData, rememberMe) {
    if (rememberMe) {
      saveSessionToStorage(authData);
    } else {
      clearSessionFromStorage();
    }
    setSession(await buildSession(authData));
  ...
  if (restoring) {
    return (
      <div style={{ minHeight: "100vh", background: C.navyDeep, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <style>{FONTS}</style>
        <Anchor size={32} color={C.gold} strokeWidth={1.6} />
      </div>
    );
  }
